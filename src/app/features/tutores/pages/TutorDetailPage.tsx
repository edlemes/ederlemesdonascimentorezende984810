...
export function TutorDetailPage() {
  ...
  const openUnlinkModal = (pet: LinkedPet) => {
    setUnlinkModal({ isOpen: true, pet })
  }

  const closeUnlinkModal = () => {
    setUnlinkModal({ isOpen: false, pet: null })
  }

  const handleConfirmUnlink = async () => {
    if (!tutor || !unlinkModal.pet) return

    const pet = unlinkModal.pet
    setUnlinkingPetId(pet.id)
    ...
    try {
      ...
      toastStore.success(
        "Pet desvinculado",
        `${pet.nome} foi desvinculado com sucesso.`,
      )
    } catch {
  ...
  if (loading) {
    return <Spinner variant="blue" size="md" fullScreen={false} />
  }
  ...
  return (
    <>
      <ConfirmModal
        ...
        message={`Deseja realmente desvincular "${unlinkModal.pet?.nome}" do tutor "${tutor?.nome}"?`}
        confirmLabel="Desvincular"
        cancelLabel="Cancelar"
        variant="danger"
        isLoading={unlinkingPetId !== null}
        ...
        <div className="relative w-full">
          ...
          <div className="relative max-w-lg mx-auto px-4">
            <div className="flex items-center justify-between mb-6">
              <button
                onClick={() => navigate("/tutores")}
                aria-label="Voltar para lista de tutores"
            ...
            <div className="bg-white rounded-3xl shadow-2xl shadow-black/30 overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {tutor.cpf && (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold bg-blue-50 text-blue-600 border border-blue-100">
                      <svg
                        className="w-4 h-4"
                        fill="none"
                        viewBox="0 0 24 24"
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold bg-orange-50 text-slate-700 border border-orange-100">
                    <svg
                      className="w-4 h-4"
                      fill="none"
                      viewBox="0 0 24 24"
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex gap-2">
                                  <Link
                                    to={`/${pet.id}`}
                                    aria-label={`Ver perfil de ${pet.nome}`}
                                    className="w-10 h-10 flex items-center justify-center bg-white hover:bg-green-50 rounded-xl shadow-sm transition-colors"
                                  >
                                    <svg
